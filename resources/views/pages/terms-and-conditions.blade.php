...
@section('title', __('terms_and_conditions.title'))
@section('meta_description', __('terms_and_conditions.meta_description'))
    ...
    <div class="min-h-screen">
        <section class="relative h-[40vh] min-h-[320px] flex items-center justify-center overflow-hidden">
            <img src="https://images.unsplash.com/photo-1450101499163-c8848c66ca85?w=1600&q=80"
                 alt="{{ __('terms_and_conditions.title') }}"
            ...
            <div class="relative z-10 text-center text-primary-foreground px-4">
                <p class="text-primary font-semibold tracking-[0.3em] uppercase text-xs mb-4">{{ __('terms_and_conditions.eyebrow') }}</p>
                <h1 class="text-4xl md:text-6xl font-bold">{{ __('terms_and_conditions.title') }}</h1>
                <x-breadcrumbs
                    variant="overlay"
                    align="center"
                    class="mt-6"
                    :items="[
                        ['label' => __('terms_and_conditions.title')],
                    ]"
                />
            </div>
        </section>
            ...
            <div class="container mx-auto max-w-5xl">
                <div class="max-w-3xl mx-auto">
                    <div class="mb-12 pb-10 border-b border-border/60">
                        @foreach(__('terms_and_conditions.intro') as $paragraph)
                            <p class="text-muted-foreground leading-relaxed mb-4 last:mb-0">{{ $paragraph }}</p>
                        @endforeach
                    </div>

                    <div class="space-y-10">
                        @foreach(__('terms_and_conditions.sections') as $index => $section)
                            <section class="group">
