...
@section('title', __('privacy_policy.title'))
@section('meta_description', __('privacy_policy.meta_description'))
    ...
    <div class="min-h-screen">
        <section class="relative h-[40vh] min-h-[320px] flex items-center justify-center overflow-hidden">
            ...
            <div class="relative z-10 text-center text-primary-foreground px-4">
                <p class="text-primary font-semibold tracking-[0.3em] uppercase text-xs mb-4">{{ __('privacy_policy.eyebrow') }}</p>
                <h1 class="text-4xl md:text-6xl font-bold">{{ __('privacy_policy.title') }}</h1>
                <x-breadcrumbs
                    variant="overlay"
                    align="center"
                    class="mt-6"
                    :items="[
                        ['label' => __('privacy_policy.title')],
                    ]"
                />
            </div>
        </section>
            ...
            <div class="container mx-auto max-w-5xl">
                <div class="max-w-3xl mx-auto">
                    <div class="mb-12 pb-10 border-b border-border/60">
                        @foreach(__('privacy_policy.intro') as $paragraph)
                            <p class="text-muted-foreground leading-relaxed mb-4 last:mb-0">{{ $paragraph }}</p>
                        @endforeach
                    </div>

                    <div class="space-y-10">
                        @foreach(__('privacy_policy.sections') as $index => $section)
                            <section class="group">
                    ...
                    <div class="mt-14 pt-10 border-t border-border/60 text-center">
                        ...
                        <p class="text-muted-foreground italic mt-1">{{ __('privacy_policy.footer_tagline') }}</p>
                    </div>
                </div>
            </div>
        </section>
